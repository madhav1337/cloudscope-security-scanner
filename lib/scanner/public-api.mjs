...
export function allowedOrigin(request) {
  ...
  if (origin) return PUBLIC_ORIGINS.has(origin) ? origin : null;

  // Browsers may omit Origin on same-origin GET requests. Sec-Fetch-Site is a
  // forbidden request header, so page JavaScript cannot forge this fallback.
  if (request.headers.get("sec-fetch-site") === "same-origin") {
    const requestOrigin = new URL(request.url).origin;
    return PUBLIC_ORIGINS.has(requestOrigin) ? requestOrigin : null;
  }
